...
*The `register` dimension migrates from `worked-off / same-field / none` to six **mechanism classes**. The whole point of the split: `same-field` bundled three very different things (a shared source, an independent convergence, and a mere analyst's resemblance) and let them read alike. Each mechanism below carries a **minimum proof** (what the graph must hold to assign it) and a **does-not-permit** (what it may never be read to license). Migration is per-edge, performed in Phase 4 as each node is absorbed. **The migration is now COMPLETE (2026-07-13): all 196 legacy edges (`worked-off`/`same-field`) were reclassified to the six mechanisms, and the legacy registers have been retired from the schema and the edge-lint — only the six mechanism classes now lint-clean.***
